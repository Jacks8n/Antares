...
It's obvious that `k` particles can be found by accessing at most `min(2k, c)` endmost elements, where `c` is the capacity before compaction.

The implementation of stream compaction is inspired by [^DVJC]. The algorithm requires a prefix sum stage referring to  [^DM].
